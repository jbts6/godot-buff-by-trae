...
func test_add_and_set_stacks_actions() -> void:
	...
	for i in range(3):
		defender["buffs"].apply_buff(defender["stats"], "buff_dummy_debuff_stackable_3", int(defender["eid"]))
	assert_eq(_count_stacks_by_buff_id(defender["buffs"], ds, "buff_dummy_debuff_stackable_3"), 3)

	# 触发一个 action：ADD_STACKS -1
	defender["buffs"].apply_buff(defender["stats"], "buff_wrapup_add_stacks_minus1", int(defender["eid"]))
	...
	defender["buffs"].apply_buff(defender["stats"], "buff_wrapup_set_stacks_zero", int(defender["eid"]))
...
func test_life_death_kill_heal_and_revive_clean_debuff() -> void:
	...
	victim["buffs"].apply_buff(victim["stats"], "buff_wrapup_on_death_heal_killer_50", int(victim["eid"]))

	# 模拟死亡事件：source_id = attacker
	var death := LifeContext.new()
	death.actor_id = int(victim["eid"])
	death.source_id = int(attacker["eid"])
	...
	victim["buffs"].apply_buff(victim["stats"], "buff_dummy_debuff_mark_1", int(victim["eid"]))
	victim["buffs"].apply_buff(victim["stats"], "buff_wrapup_on_revive_clean_debuff", int(victim["eid"]))
	assert_eq(_count_instances_by_buff_id(victim["buffs"], ds, "buff_dummy_debuff_mark_1"), 1)

	var revive := LifeContext.new()
	revive.actor_id = int(victim["eid"])
	revive.source_id = -1
